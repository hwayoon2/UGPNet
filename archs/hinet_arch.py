...
GAUSSIAN_BLUR = transforms.GaussianBlur(kernel_size=(7, 13), sigma=(1, 2))
...
class HINet(nn.Module):
    ...
    def forward(self, x, return_f=False, modified=None,return_f_img=False):

        image = x
        #stage 1
        if modified is None: #image and modified feature with 32channel is input
            x1 = self.conv_01(image)
        else: #image or modified feature with 3channel is input
            x1 = modified
